...
def measure_single_translate(model, filename, sentencepiece_model='tokenizer/bpe.cs-en.32000.model', max_iteration=10):
    ...
    with open(filename) as file:
        ...
        for line in lines:
            ...
            if encoded_tokens:
                ...
                start = time.time()
                translated_string = model.translate(encoded_string, beam=1, iter_decode_max_iter=max_iteration)
                end = time.time()
...
if __name__ == '__main__':
    ...
    model = load_model(checkpoint_dir=args.checkpoint_dir, user_dir=args.user_dir)
    #print(f'BEAM VALUE {int(args.beam)}')
    for mi in [1,2,3,4,5,10,20]:
        print(f'MAX ITERATION {mi}')
        measure_single_translate(filename=args.file2translate, model=model, max_iteration=mi)
